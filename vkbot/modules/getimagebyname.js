...
const telegramRequest = async (name, dirname = "") => {
    return new Promise((resolve, reject) => {
        ...
        if (!fs.existsSync(path2element)) {
            resolve([])  // ← раньше тут был просто return, промис зависал
            return
        }
            ...
            try {
                if (jpegfiles.length < 3) {
                    ...
                    for (let index = 0; index < xlsxfiles.length; index++) {
                        ...
                        result.push((index + 1) + ".jpeg")
                    }
                    resolve(result.map(fileName => tempdir + `${dirname}/${name}/` + fileName))
                } else {
                    resolve(jpegfiles.map(fileName => tempdir + `${dirname}/${name}/` + fileName))
                }
            } catch (error) {
                console.error('[getimagebyname] ошибка генерации:', error)
                reject(error)
            }
        })
    })
